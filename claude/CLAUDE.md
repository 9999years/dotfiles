...
Prefer `rg` over `grep`, because `rg` will automatically skip over `.git` directories and other gitignore'd files; in particular, this will keep `grep` from searching enormous build directories.
